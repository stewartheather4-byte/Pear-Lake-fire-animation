...
def firms_url(
    ...
    return (
        f"{FIRMS_BASE}/{map_key}/{source}/"
        f"{area}/{days}/{start.isoformat()}"
    )
...
def fetch_chunk(
    ...
    if not required.issubset(df.columns):
        raise RuntimeError(
            f"FIRMS response missing columns: "
...
def fetch_missing_data(
    ...
    if (
        ...
        fetch_start = max(
            START_DATE,
            latest_cached - timedelta(days=2),
        )
...
def normalize_and_save(
    ...
    frames = [
        df
        for df in (cached, new_data)
        if not df.empty
    ]

    if not frames:
        raise RuntimeError(
            "No FIRMS data were available. "
            "Check the FIRMS_MAP_KEY and GitHub Actions log."
        )
...
def make_frame(
    ...
    # Wide landscape frame so the map fills the display.
    fig, ax = plt.subplots(
        figsize=(15, 10)
    )
    ...
    # IMPORTANT:
    # "auto" lets the map fill the available display.
    # "equal" caused the old map to be squeezed into
    # a narrow strip at the top.
    ax.set_aspect(
        "auto"
    )
    ...
    # Title stays INSIDE the map so it cannot be cut off.
    ax.text(
        0.5,
        0.985,
        FIRE_TITLE,
    ...
    # Description stays inside the map,
    # preventing a large white area below it.
    ax.text(
        0.5,
        0.020,
        FIRE_SUBTITLE
        + "\n"
        + (
            "Red = cumulative satellite thermal detections; "
            "not an official wildfire perimeter."
        ),
        transform=ax.transAxes,
        fontsize=8.5,
        ha="center",
    ...
    # Fill almost the entire PNG frame.
    fig.subplots_adjust(
        left=0.055,
        right=0.995,
        top=0.995,
    ...
    # That was contributing to inconsistent image sizing.
    fig.savefig(
        output_path,
        dpi=FRAME_DPI,
        facecolor="white",
...
def build_frames(
    ...
    shutil.copy2(
        frame_paths[-1],
        ROOT / "pear_lake_latest.png",
    )

    return frame_paths


def even_frame_array(
    path: Path,
) -> np.ndarray:
...
def build_mp4(
    ...
    with imageio.get_writer(
        ...
        for path in frame_paths:
            ...
            writer.append_data(
                even_frame_array(path)
            )

        last = even_frame_array(
            frame_paths[-1]
        )
...
def build_website(
    ...
    log(
        "Building GitHub Pages animation..."
    )
    ...
    manifest = {
        "name": "Pear Lake Wildfire Animation",
        "short_name": "Pear Lake Fire",
        "start_url": "./",
        "display": "standalone",
...
<img
    id="frame"
    alt="Pear Lake wildfire animation"
>
...
def update_stop_state(
    ...
    if not refresh_ok:
        ...
        log(
            "FIRMS refresh was incomplete; "
            "no-expansion counter was NOT advanced."
        )
    ...
    if expanded:
        ...
        log(
            f"Expansion detected on "
            f"completed day {evaluation_day}."
        )
        ...
        log(
            f"No meaningful expansion on "
            f"{evaluation_day}. "
            f"Consecutive completed days: "
    ...
    if (
        ...
        STOP_FLAG.write_text(
            (
                ...
                f"{state['no_expansion_days']} "
                f"consecutive completed days.\n"
                f"Last evaluated day: "
                f"{evaluation_day}\n"
                f"Tolerance: "
                f"{EXPANSION_TOLERANCE_METERS} metres.\n"
                "This is an animation stop rule "
                "based on FIRMS hotspots, "
