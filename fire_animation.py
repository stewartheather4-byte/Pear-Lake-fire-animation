...
def make_frame(df: pd.DataFrame, frame_date: date, output_path: Path) -> None:
    ...
    fig.text(
        0.5,
        0.012,
        "Red = cumulative satellite thermal detections. FIRMS hotspots are not an official wildfire perimeter.",
        ha="center",
        fontsize=8,
    )
...
def build_website(frame_paths: list[Path]) -> None:
    """
    Builds the GitHub Pages web animation.
    It plays once, then the final frame stays displayed until the page is closed.
    """
...
<body>
<img id="frame" alt="Pear Lake wildfire animation">
<div id="controls">
...
function showNext() {{
    ...
    if (i < frames.length - 1) {{
        ...
        stopTimer();
        // IMPORTANT:
        // No new timer is created here.
        // The final frame stays on the screen until the user closes the page
        // or taps Replay.
    }}
}}
...
"""
    ...
    # Root copy for a branch/folder-style Pages setup.
    root_html = html.replace('frames/', 'site/frames/')
    (ROOT / "index.html").write_text(root_html, encoding="utf-8")

    log("Created website: site/index.html")
